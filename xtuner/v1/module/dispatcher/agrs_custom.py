...
use_custom_rs = int(os.getenv("USE_CUSTOM_RS_IN_DISPATCHER", 0))
use_custom_ag = int(os.getenv("USE_CUSTOM_AG_IN_DISPATCHER", 0))
    ...
    def backward(
        ...
        with torch.cuda.stream(ctx.comm_stream):
            ...
            if use_custom_rs:
                ...
                # )
                # print(f"{combined_grad_output = }, {combined_grad_output_our = }")
...
class _AsyncCombine(Function):
    ...
    def forward(
        ...
        rs_fwd += 1
        with torch.no_grad():
            # Barrier On Compute stream
            if rs_fwd % int(os.getenv("INTRA_LAYER_MICRO_BATCHR", 1)) == 0 and int(os.getenv("BARRIER_DISPATCHER_ON_COMP", 0)):
                barrier_tensor = torch.randn(1, device = hidden_states.device)
                with torch.cuda.stream(torch.cuda.default_stream()):
                    dist.all_reduce(barrier_tensor, op=dist.ReduceOp.AVG)
        ...
        with torch.cuda.stream(comm_stream):
            ...
            if use_custom_rs:
                ...
                # )
                # print(f"{combined_hidden_states_our = }, {combined_hidden_states = }")
