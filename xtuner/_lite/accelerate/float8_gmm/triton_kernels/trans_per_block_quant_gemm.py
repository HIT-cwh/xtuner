...
def trans_per_block_quant_gemm_kernel(
        input_ptr, output_ptr, output_scales_ptr,
        stride_in_m: tl.constexpr, stride_in_n: tl.constexpr, stride_out_n,
        stride_out_m: tl.constexpr, stride_out_scale_n,
        stride_out_scale_m: tl.constexpr, M: tl.constexpr, N: tl.constexpr,
        fmax: tl.constexpr, fmin: tl.constexpr, 
    ...
    mask_m = offs_m < M
    mask_n = offs_n < N
    input_offset = input_ptr + offs_m[:, None] * stride_in_m + offs_n[
        None, :] * stride_in_n
    output_offset = output_ptr + offs_n[
        None, :] * stride_out_n + offs_m[:, None] * stride_out_m
    output_scale_offset = output_scales_ptr + m * stride_out_scale_m + n * stride_out_scale_n
    input_block = tl.load(input_offset,
    ...
    input_block = tl.clamp(input_block, fmin,
                           fmax).to(output_ptr.dtype.element_ty)
    tl.store(output_offset,
             input_block,
             mask=mask_m[:, None] & mask_n[None, :])
    tl.store(output_scale_offset, output_block_scale)


# @triton_op("myfp8::trans_per_block_quant_gemm", mutates_args={})
# def trans_per_block_quant_gemm(input_tensor: torch.Tensor, group_size: int=128, dtype: torch.dtype = torch.float8_e4m3fn) -> Tuple[torch.Tensor, torch.Tensor]:
#     """Do per-tile quantization along the first dimension. And finally transpose it.
#     Args:
...
#     output_scales = input_tensor.new_empty((triton.cdiv(N, group_size), triton.cdiv(M, group_size)), dtype=torch.float32)
#     grid = (triton.cdiv(M, group_size) * triton.cdiv(N, group_size), )
#     wrap_triton(trans_per_block_quant_gemm_kernel)[grid](input_tensor,
#                                  output_tensor,
#                                  output_scales,
...
@torch.library.custom_op("moe::trans_per_block_quant_gemm", mutates_args=())
def trans_per_block_quant_gemm(input_tensor: torch.Tensor, group_size: int=128, dtype: torch.dtype = torch.float8_e4m3fn) -> Tuple[torch.Tensor, torch.Tensor]:
    M, N = input_tensor.shape
    finfo = torch.finfo(dtype)
    fmin = finfo.min
    ...
    output_scales = input_tensor.new_empty((triton.cdiv(N, group_size), triton.cdiv(M, group_size)), dtype=torch.float32)
    grid = (triton.cdiv(M, group_size) * triton.cdiv(N, group_size), )
    trans_per_block_quant_gemm_kernel[grid](input_tensor,
                                 output_tensor,
                                 output_scales,
                                 input_tensor.stride(0),
                                 input_tensor.stride(1),
...
def _(input_tensor: torch.Tensor, group_size: int=128, dtype: torch.dtype = torch.float8_e4m3fn) -> Tuple[torch.Tensor, torch.Tensor]:
    M, N = input_tensor.shape
    output_tensor = input_tensor.new_empty((N, M), dtype=dtype)
    output_scales = input_tensor.new_empty((triton.cdiv(N, group_size), triton.cdiv(M, group_size)), dtype=torch.float32)
    return output_tensor, output_scales
