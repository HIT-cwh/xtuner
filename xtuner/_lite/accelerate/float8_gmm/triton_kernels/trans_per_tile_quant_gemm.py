...
# @triton_op("myfp8::trans_per_tile_quant_gemm", mutates_args={})
# def trans_per_tile_quant_gemm(input_tensor: torch.Tensor, group_size: int=128, dtype: torch.dtype = torch.float8_e4m3fn) -> Tuple[torch.Tensor, torch.Tensor]:
...
#     grid = lambda meta: (triton.cdiv(M, meta['GROUP_SIZE']) * triton.cdiv(
#         N, meta['BLOCK_N']), )
#     wrap_triton(trans_per_tile_quant_gemm_kernel)[grid](input_tensor,
#                                  output_tensor,
#                                  output_scales,
...
@torch.library.custom_op("moe::trans_per_tile_quant_gemm", mutates_args=())
def trans_per_tile_quant_gemm(input_tensor: torch.Tensor, group_size: int=128, dtype: torch.dtype = torch.float8_e4m3fn) -> Tuple[torch.Tensor, torch.Tensor]:
    """Do per-tile quantization along the first dimension. And finally transpose it.
    Args:
        input_tensor (Tensor): The input quantized tensor. Shape [M, N]. It is
    ...
    grid = lambda meta: (triton.cdiv(M, meta['GROUP_SIZE']) * triton.cdiv(
        N, meta['BLOCK_N']), )
    trans_per_tile_quant_gemm_kernel[grid](input_tensor,
                                 output_tensor,
                                 output_scales,
                                 input_tensor.stride(0),
                                 input_tensor.stride(1),
