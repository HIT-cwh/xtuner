...
class FSDPParamGroup:
    ...
    def pre_forward(
        ...
        if is_bar:
            barrier_tensor = torch.randn(1, device = self.device)
            with torch.cuda.stream(torch.cuda.default_stream()):
                dist.all_reduce(barrier_tensor, group = self._all_gather_process_group, op=dist.ReduceOp.AVG)
    ...
    def post_backward(self, *unused: Any):
        ...
        with record_function(self._with_fqn("FSDP::post_backward_reduce")):
            ...
            layer_num = 1 if self._module_fqn is None else self._module_fqn
